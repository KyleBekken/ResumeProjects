...
(* SCANNER. Lexical scanner for Lisp, also ignores comments. *)
...
(*Initialize CH by reading the NEXT CHAR, and return next token. *)
	...
	let makeParser path = 
		...
		let rec nextThing () =
			...
			with CloseParenToken -> 
					raise (Can'tParse "Unmatched close parenthesis.")|
		...
		and nextThings () =
			...
			with CloseParenToken -> 
				...
				EndToken -> raise (Can'tParse "Unexpected end of file.") |
				_ -> 
					let evalFirst = nextThing ()
					in Cons(evalFirst, nextThings ())
	in nextThing	
	end;;
